...
public class BinaryTreePanel extends JPanel {
    ...
    public void paintComponent(final Graphics g) {
        ...
        final StringBuilder preDatas = new StringBuilder("先根遍历：");
        tree.traversePreorder(new BinaryTreeNode.Visitor() {
            @Override
            public <E> void visit(BinaryTreeNode<E> node) {
                preDatas.append(node.getData()).append(" ");
            }
        });
        final StringBuilder inDatas = new StringBuilder("中根遍历：");
        tree.traverseInorder(new BinaryTreeNode.Visitor() {
            @Override
            public <E> void visit(BinaryTreeNode<E> node) {
                inDatas.append(node.getData()).append(" ");
            }
        });
        final StringBuilder postDatas = new StringBuilder("后根遍历：");
        tree.traversePostorder(new BinaryTreeNode.Visitor() {
            @Override
            public <E> void visit(BinaryTreeNode<E> node) {
                postDatas.append(node.getData()).append(" ");
            }
        });
        final StringBuilder levelDatas = new StringBuilder("层次遍历：");
        tree.traverseLevelOrder(new BinaryTreeNode.Visitor() {
            @Override
            public <E> void visit(BinaryTreeNode<E> node) {
                levelDatas.append(node.getData()).append(" ");
            }
        });
        g.drawString(preDatas.toString(), 10,110);
        g.drawString(inDatas.toString(), 10,130);
        g.drawString(postDatas.toString(), 10,150);
        g.drawString(levelDatas.toString(), 10,170);
    }
